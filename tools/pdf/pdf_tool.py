"""PDF 工具箱 - 合并 & 拆分 & 旋转

提供 PDF 文件的合并、拆分和旋转功能，支持拖拽操作。
优化版本：更好的代码结构、性能和用户体验。
"""

import os
import queue
import re
import threading
import tkinter as tk
from collections import OrderedDict
from concurrent.futures import ThreadPoolExecutor
from tkinter import filedialog, messagebox
from typing import TYPE_CHECKING, Any, Callable, Optional
from dataclasses import dataclass

from PyPDF2 import PdfMerger, PdfReader, PdfWriter
from tkinterdnd2 import DND_FILES, TkinterDnD
import ttkbootstrap as ttk
from ttkbootstrap import Frame, Labelframe as LabelFrame

if TYPE_CHECKING:
    pass

try:
    import fitz
    from PIL import Image, ImageTk
    HAS_FITZ = True
except ImportError:
    HAS_FITZ = False


# =============================================================================
# 配置常量
# =============================================================================

@dataclass(frozen=True)
class Config:
    """应用配置"""
    WINDOW_TITLE: str = "PDF 工具箱"
    WINDOW_SIZE: str = "1080x800"
    WINDOW_MIN_SIZE: tuple = (900, 650)
    SUPPORTED_FORMAT: str = ".pdf"

    # 颜色
    COLOR_PRIMARY: str = "#2563eb"
    COLOR_SUCCESS: str = "#10b981"
    COLOR_WARNING: str = "#f59e0b"
    COLOR_DANGER: str = "#ef4444"
    COLOR_INFO: str = "#3b82f6"
    COLOR_BG_LIGHT: str = "#f8fafc"
    COLOR_BG_CARD: str = "#ffffff"
    COLOR_BORDER: str = "#e2e8f0"
    COLOR_TEXT_PRIMARY: str = "#1e293b"
    COLOR_TEXT_SECONDARY: str = "#64748b"
    COLOR_HOVER: str = "#e0f7fa"
    COLOR_SELECTED: str = "#dbeafe"

    # 缓存
    MAX_CACHE_PAGES: int = 80
    THUMBNAIL_CACHE_SIZE: int = 150

    # 缩略图
    THUMBNAIL_SIZE: tuple = (100, 140)
    THUMBNAIL_COLUMNS: int = 5
    THUMBNAIL_RENDER_WORKERS: int = 4

    # 预览
    PREVIEW_DPI: int = 120
    PREVIEW_MAX_WIDTH: int = 500

    # 左侧面板宽度
    LEFT_PANEL_WIDTH: int = 400


CONFIG = Config()


def _format_size(size_bytes: int) -> str:
    """格式化文件大小"""
    if size_bytes >= 1_048_576:
        return f"{size_bytes / 1_048_576:.1f} MB"
    return f"{size_bytes / 1024:.0f} KB"


# =============================================================================
# 缓存管理
# =============================================================================

class PageCache:
    """页面渲染缓存管理器，使用 LRU 策略"""

    def __init__(self, max_pages: int = CONFIG.MAX_CACHE_PAGES):
        self._cache: OrderedDict = OrderedDict()
        self._max_pages = max_pages
        self._lock = threading.Lock()

    def get(self, filepath: str, page_num: int, dpi: int = 100):
        """获取缓存的页面图像"""
        key = (filepath, dpi)
        with self._lock:
            if key in self._cache and page_num in self._cache[key]:
                self._cache.move_to_end(key)
                return self._cache[key][page_num]
        return None

    def set(self, filepath: str, page_num: int, image, dpi: int = 100):
        """缓存页面图像"""
        key = (filepath, dpi)
        with self._lock:
            if key not in self._cache:
                self._cache[key] = OrderedDict()
            self._cache[key][page_num] = image
            self._cache.move_to_end(key)
            self._evict()

    def _evict(self):
        """LRU 淘汰策略"""
        total = sum(len(v) for v in self._cache.values())
        while total > self._max_pages and self._cache:
            oldest_key = next(iter(self._cache))
            oldest_pages = self._cache[oldest_key]
            if oldest_pages:
                oldest_pages.popitem(last=False)
                total -= 1
            if not oldest_pages:
                self._cache.pop(oldest_key)

    def clear(self):
        """清空缓存"""
        with self._lock:
            self._cache.clear()

    def remove_file(self, filepath: str):
        """移除指定文件的所有缓存"""
        with self._lock:
            keys_to_remove = [k for k in self._cache if k[0] == filepath]
            for k in keys_to_remove:
                self._cache.pop(k, None)


# 全局缓存实例
_page_cache = PageCache()


# =============================================================================
# UI 组件基类
# =============================================================================

class DropZone(Frame):
    """可拖拽的文件放置区域"""

    def __init__(self, parent, text: str = "📎 拖拽 PDF 文件到这里",
                 on_drop: Optional[Callable[[list[str]], None]] = None, **kwargs: Any):
        super().__init__(parent, padding=20, relief="groove", borderwidth=2, **kwargs)

        self.on_drop = on_drop
        self._normal_style = "default"
        self._hover_style = "success"

        self.label = ttk.Label(
            self, text=text,
            font=("微软雅黑", 11), bootstyle="secondary", anchor="center"
        )
        self.label.pack(fill=tk.BOTH, expand=True)

        # tkinterdnd2 方法 - 类型检查器无法识别
        self.drop_target_register(DND_FILES)  # type: ignore[attr-defined]
        self.dnd_bind("<<Drop>>", self._on_drop)  # type: ignore[attr-defined]
        self.dnd_bind("<<DropEnter>>", self._on_enter)  # type: ignore[attr-defined]
        self.dnd_bind("<<DropLeave>>", self._on_leave)  # type: ignore[attr-defined]

    def _on_drop(self, event: Any) -> None:
        self.configure(bootstyle=self._normal_style)
        self.label.configure(bootstyle="secondary")
        if self.on_drop:
            files = self._parse_files(event.data)
            self.on_drop(files)

    def _on_enter(self, _event: Any) -> None:
        self.configure(bootstyle=self._hover_style)
        self.label.configure(bootstyle="success")

    def _on_leave(self, _event: Any) -> None:
        self.configure(bootstyle=self._normal_style)
        self.label.configure(bootstyle="secondary")

    def _parse_files(self, data: Any) -> list[str]:
        """解析拖拽的文件列表"""
        files = self.tk.splitlist(data)
        return [os.path.abspath(f) for f in files
                if os.path.isfile(f) and f.lower().endswith(CONFIG.SUPPORTED_FORMAT)]

    def set_text(self, text: str) -> None:
        self.label.configure(text=text)


class ProgressArea(Frame):
    """进度条区域组件"""

    def __init__(self, parent, style: str = "success-striped", **kwargs: Any):
        super().__init__(parent, **kwargs)

        self.progress_bar = ttk.Progressbar(self, bootstyle=style)
        self.progress_bar.pack(fill=tk.X)

        self.label = ttk.Label(
            self, text="", bootstyle="secondary",
            font=("微软雅黑", 9)
        )
        self.label.pack(fill=tk.X)

        self.pack_forget()

    def show(self) -> None:
        self.progress_bar["value"] = 0
        self.label["text"] = ""
        self.pack(fill=tk.X, pady=(5, 0))

    def hide(self) -> None:
        self.pack_forget()

    def update(self, pct: int, text: Optional[str] = None) -> None:
        self.progress_bar["value"] = pct
        if text:
            self.label["text"] = text
        else:
            self.label["text"] = f"处理中... {pct}%"


class SectionHeader(Frame):
    """区块标题组件"""

    def __init__(self, parent, title: str, subtitle: Optional[str] = None, **kwargs: Any):
        super().__init__(parent, **kwargs)

        ttk.Label(
            self, text=title,
            font=("微软雅黑", 13, "bold"), bootstyle="primary"
        ).pack(anchor="w")

        if subtitle:
            ttk.Label(
                self, text=subtitle,
                font=("微软雅黑", 9), bootstyle="secondary"
            ).pack(anchor="w", pady=(2, 0))


# =============================================================================
# PDF 预览面板
# =============================================================================

class PDFPreviewPanel(LabelFrame):
    """分页式 PDF 预览面板"""

    def __init__(self, parent, app: "PDFToolApp", max_width: int = CONFIG.PREVIEW_MAX_WIDTH,
                 dpi: int = CONFIG.PREVIEW_DPI, **kwargs: Any):
        self.app = app
        self.dpi = dpi
        self.max_width = max_width

        self._filepath = ""
        self._total_pages = 0
        self._current_page = 0
        self._target_page = 0
        self._rendering = False
        self._photo = None
        self._doc = None

        super().__init__(parent, text="  📄 PDF 预览  ", **kwargs)
        self._build_ui()

    def _build_ui(self) -> None:
        # 导航栏
        self.nav = ttk.Frame(self)
        self.nav.pack(fill=tk.X, pady=5)

        self.prev_btn = ttk.Button(
            self.nav, text="◀ 上一页", bootstyle="secondary-outline",
            command=self.go_prev, state=tk.DISABLED, width=10
        )
        self.prev_btn.pack(side=tk.LEFT, padx=5)

        self.page_label = ttk.Label(
            self.nav, text="请选择 PDF 文件",
            bootstyle="info", font=("微软雅黑", 10, "bold"), anchor="center"
        )
        self.page_label.pack(side=tk.LEFT, fill=tk.X, expand=True)

        self.next_btn = ttk.Button(
            self.nav, text="下一页 ▶", bootstyle="secondary-outline",
            command=self.go_next, state=tk.DISABLED, width=10
        )
        self.next_btn.pack(side=tk.RIGHT, padx=5)

        # Canvas 显示区域
        canvas_border = ttk.Frame(self, bootstyle="default", relief="solid", borderwidth=1)
        canvas_border.pack(fill=tk.BOTH, expand=True, pady=5)

        self.canvas = tk.Canvas(
            canvas_border, bg=CONFIG.COLOR_BG_LIGHT,
            width=self.max_width, height=400,
            highlightthickness=0
        )
        self.canvas.pack(fill=tk.BOTH, expand=True)

        # 占位符
        self.placeholder = ttk.Label(
            canvas_border, text="📄 选择 PDF 文件后在此处预览",
            bootstyle="secondary", font=("微软雅黑", 11), anchor="center"
        )
        self.placeholder.pack(fill=tk.BOTH, expand=True)

        # 加载提示
        self.loading_label = ttk.Label(
            canvas_border, text="", bootstyle="warning",
            font=("微软雅黑", 10)
        )
        self.loading_label.pack_forget()

        self.nav.pack_forget()

    def set_file(self, filepath: str):
        if not HAS_FITZ:
            self.placeholder.config(text="请安装 PyMuPDF：pip install PyMuPDF")
            return
        if filepath == self._filepath:
            return

        self._close_doc()
        self._filepath = filepath
        self._photo = None

        try:
            self._doc = fitz.open(filepath)
            self._total_pages = self._doc.page_count
        except Exception:
            self._total_pages = 0

        if self._total_pages == 0:
            self.placeholder.config(text="无法打开此 PDF 文件")
            self.page_label.config(text="")
            self.prev_btn.config(state=tk.DISABLED)
            self.next_btn.config(state=tk.DISABLED)
            return

        self._current_page = 1
        self.placeholder.pack_forget()
        self.loading_label.pack_forget()
        self.nav.pack(fill=tk.X, pady=3)
        self._render_page(1)

    def go_prev(self) -> None:
        if self._current_page > 1:
            self._render_page(self._current_page - 1)

    def go_next(self) -> None:
        if self._current_page < self._total_pages:
            self._render_page(self._current_page + 1)

    def clear(self) -> None:
        self._close_doc()
        self._filepath = ""
        self._total_pages = 0
        self._current_page = 0
        self._target_page = 0
        self._photo = None
        self.canvas.delete("all")
        self.prev_btn.config(state=tk.DISABLED)
        self.next_btn.config(state=tk.DISABLED)
        self.page_label.config(text="请选择 PDF 文件")
        self.loading_label.pack_forget()
        self.placeholder.config(text="📄 选择 PDF 文件后在此处预览")
        self.placeholder.pack(fill=tk.BOTH, expand=True)
        self.nav.pack_forget()

    def _close_doc(self) -> None:
        if self._doc:
            try:
                self._doc.close()
            except Exception:
                pass
            self._doc = None

    def _render_page(self, page_num: int) -> None:
        if page_num < 1 or page_num > self._total_pages or not self._filepath:
            return

        self._target_page = page_num
        cached = _page_cache.get(self._filepath, page_num, self.dpi)
        if cached:
            self._display_image(cached, page_num)
            return

        self._rendering = True
        self.canvas.delete("all")
        self.placeholder.pack_forget()
        self.loading_label.config(text="正在渲染页面...")
        self.loading_label.pack(fill=tk.X, pady=5)
        self.prev_btn.config(state=tk.DISABLED)
        self.next_btn.config(state=tk.DISABLED)
        self.page_label.config(text=f"Loading {page_num} / {self._total_pages}")

        filepath = self._filepath
        dpi = self.dpi

        def _worker() -> None:
            try:
                doc = fitz.open(filepath)
                page = doc[page_num - 1]
                pix = page.get_pixmap(dpi=dpi)
                img = Image.frombytes("RGB", (pix.width, pix.height), pix.samples)
                doc.close()
                self.app._dispatch(self._on_render_complete, page_num, img)
            except Exception as e:
                self.app._dispatch(self._on_render_error, str(e))

        threading.Thread(target=_worker, daemon=True).start()

    def _on_render_complete(self, page_num: int, image: Any) -> None:
        if self._target_page != page_num:
            return
        self._rendering = False
        _page_cache.set(self._filepath, page_num, image, self.dpi)
        self._display_image(image, page_num)

    def _on_render_error(self, error: str) -> None:
        self._rendering = False
        self.loading_label.pack_forget()
        self.placeholder.config(text=f"渲染失败：{error}")
        self.placeholder.pack(fill=tk.BOTH, expand=True)
        self.prev_btn.config(state=tk.NORMAL)
        self.next_btn.config(state=tk.NORMAL)

    def _display_image(self, image: Any, page_num: int) -> None:
        self.loading_label.pack_forget()

        cw = self.canvas.winfo_width()
        ch = self.canvas.winfo_height()
        if cw < 10 or ch < 10:
            cw, ch = self.max_width, 400

        iw, ih = image.size
        scale = min(cw / iw, ch / ih, 1.0)
        if scale < 1.0:
            nw, nh = max(1, int(iw * scale)), max(1, int(ih * scale))
            image = image.resize((nw, nh), Image.Resampling.LANCZOS)

        self._photo = ImageTk.PhotoImage(image)
        self.canvas.delete("all")
        self.canvas.create_image(cw / 2, ch / 2, image=self._photo, anchor="center")

        self._current_page = page_num
        self.page_label.config(text=f"第 {page_num} / {self._total_pages} 页")
        self.prev_btn.config(state=tk.NORMAL if page_num > 1 else tk.DISABLED)
        self.next_btn.config(state=tk.NORMAL if page_num < self._total_pages else tk.DISABLED)


class RotationPreviewPanel(PDFPreviewPanel):
    """支持旋转预览的面板"""

    def __init__(self, parent, app: "PDFToolApp", **kwargs: Any):
        super().__init__(parent, app, **kwargs)
        self._preview_rotation = 0

    def set_preview_rotation(self, angle: int) -> None:
        self._preview_rotation = angle
        if self._current_page > 0:
            self._render_page(self._current_page)

    def _display_image(self, image: Any, page_num: int) -> None:
        if self._preview_rotation != 0:
            image = image.rotate(-self._preview_rotation, expand=True)
        super()._display_image(image, page_num)


# =============================================================================
# 缩略图网格
# =============================================================================

class PageThumbnailGrid(LabelFrame):
    """缩略图网格面板，支持多选页面"""

    def __init__(self, parent, app: "PDFToolApp",
                 on_selection_change: Optional[Callable[[set[int]], None]] = None,
                 thumbnail_size: tuple = CONFIG.THUMBNAIL_SIZE,
                 columns: int = CONFIG.THUMBNAIL_COLUMNS, **kwargs: Any):
        self.app = app
        self.on_selection_change = on_selection_change
        self.thumbnail_size = thumbnail_size
        self.columns = columns

        self._filepath = ""
        self._total_pages = 0
        self._selected_pages: set[int] = set()
        self._thumbnails: dict[int, ImageTk.PhotoImage] = {}
        self._thumbnail_frames: dict[int, Frame] = {}
        self._executor = ThreadPoolExecutor(max_workers=CONFIG.THUMBNAIL_RENDER_WORKERS)
        self._futures: list = []

        super().__init__(parent, text="  📑 页面选择  ", **kwargs)
        self._build_ui()

    def _build_ui(self) -> None:
        # 工具栏
        toolbar = ttk.Frame(self)
        toolbar.pack(fill=tk.X, pady=5)

        ttk.Button(toolbar, text="✅ 全选", command=self.select_all,
                   width=8, bootstyle="info-outline").pack(side=tk.LEFT, padx=3)
        ttk.Button(toolbar, text="🔄 反选", command=self.invert_selection,
                   width=8, bootstyle="secondary-outline").pack(side=tk.LEFT, padx=3)
        ttk.Button(toolbar, text="🗑 清空", command=self.clear_selection,
                   width=8, bootstyle="secondary-outline").pack(side=tk.LEFT, padx=3)

        self.selection_label = ttk.Label(
            toolbar, text="已选: 0 页",
            bootstyle="info", font=("微软雅黑", 10, "bold")
        )
        self.selection_label.pack(side=tk.RIGHT, padx=5)

        # 滚动区域
        canvas_container = ttk.Frame(self)
        canvas_container.pack(fill=tk.BOTH, expand=True)

        self.canvas = tk.Canvas(canvas_container, bg=CONFIG.COLOR_BG_LIGHT, highlightthickness=0)
        v_scroll = ttk.Scrollbar(canvas_container, orient="vertical", command=self.canvas.yview)
        self.canvas.configure(yscrollcommand=v_scroll.set)

        self.canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        v_scroll.pack(side=tk.RIGHT, fill=tk.Y)

        self.grid_frame = ttk.Frame(self.canvas)
        self.canvas_window = self.canvas.create_window((0, 0), window=self.grid_frame, anchor="nw")

        # 占位符
        self.placeholder = ttk.Label(
            self.grid_frame, text="📄 选择 PDF 文件后显示页面缩略图",
            bootstyle="secondary", font=("微软雅黑", 11)
        )
        self.placeholder.pack(pady=50)

        # 加载状态
        self.loading_label = ttk.Label(
            self.grid_frame, text="", bootstyle="warning",
            font=("微软雅黑", 10)
        )

        # 绑定事件
        self.grid_frame.bind("<Configure>", self._on_frame_configure)
        self.canvas.bind("<Configure>", self._on_canvas_configure)
        self.canvas.bind("<MouseWheel>", self._on_mousewheel)

    def _on_frame_configure(self, _event: Any) -> None:
        self.canvas.configure(scrollregion=self.canvas.bbox("all"))

    def _on_canvas_configure(self, event: Any) -> None:
        self.canvas.itemconfig(self.canvas_window, width=event.width)

    def _on_mousewheel(self, event: Any) -> None:
        self.canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")

    def set_file(self, filepath: str) -> None:
        if not HAS_FITZ:
            self.placeholder.config(text="请安装 PyMuPDF：pip install PyMuPDF")
            return

        self.clear()
        self._filepath = filepath

        try:
            doc = fitz.open(filepath)
            self._total_pages = doc.page_count
            doc.close()
        except Exception:
            self.placeholder.config(text="无法打开此 PDF 文件")
            return

        if self._total_pages == 0:
            self.placeholder.config(text="PDF 文件没有页面")
            return

        self.placeholder.pack_forget()
        self._build_grid()
        self._start_parallel_render()

    def _build_grid(self) -> None:
        for i in range(self._total_pages):
            page_num = i + 1
            row, col = i // self.columns, i % self.columns

            frame = Frame(self.grid_frame, relief="solid", borderwidth=1, padding=3)
            frame.grid(row=row, column=col, padx=4, pady=4, sticky="nsew")

            placeholder = ttk.Label(
                frame, text=f"📄 {page_num}",
                font=("微软雅黑", 9), bootstyle="secondary"
            )
            placeholder.pack(expand=True)

            label = ttk.Label(
                frame, text=f"第 {page_num} 页",
                font=("微软雅黑", 8), bootstyle="secondary"
            )
            label.pack()

            frame.bind("<Button-1>", lambda e, p=page_num: self.toggle_page(p))
            placeholder.bind("<Button-1>", lambda e, p=page_num: self.toggle_page(p))

            self._thumbnail_frames[page_num] = frame

    def _start_parallel_render(self) -> None:
        """并行渲染缩略图"""
        for page_num in range(1, self._total_pages + 1):
            future = self._executor.submit(self._render_thumbnail, page_num)
            future.add_done_callback(lambda f, p=page_num: self._on_thumbnail_done(p, f))
            self._futures.append(future)

    def _render_thumbnail(self, page_num: int) -> Optional[Any]:
        """渲染单个缩略图"""
        try:
            doc = fitz.open(self._filepath)
            page = doc[page_num - 1]
            zoom = min(self.thumbnail_size[0] / page.rect.width,
                       self.thumbnail_size[1] / page.rect.height)
            mat = fitz.Matrix(zoom, zoom)
            pix = page.get_pixmap(matrix=mat)
            img = Image.frombytes("RGB", (pix.width, pix.height), pix.samples)
            doc.close()
            return img
        except Exception:
            return None

    def _on_thumbnail_done(self, page_num: int, future: Any) -> None:
        """缩略图渲染完成回调"""
        try:
            image = future.result()
            if image and page_num in self._thumbnail_frames:
                self.app._dispatch(self._update_thumbnail, page_num, image)
        except Exception:
            pass

    def _update_thumbnail(self, page_num: int, image: Any) -> None:
        """更新缩略图显示"""
        if page_num not in self._thumbnail_frames:
            return

        frame = self._thumbnail_frames[page_num]
        for widget in frame.winfo_children():
            widget.destroy()

        photo = ImageTk.PhotoImage(image)
        self._thumbnails[page_num] = photo

        img_label = ttk.Label(frame, image=photo)
        img_label.pack(expand=True)
        img_label.bind("<Button-1>", lambda e, p=page_num: self.toggle_page(p))

        label = ttk.Label(frame, text=f"第 {page_num} 页",
                          font=("微软雅黑", 8), bootstyle="secondary")
        label.pack()

        if page_num in self._selected_pages:
            frame.configure(bootstyle="success")

    def toggle_page(self, page_num: int) -> None:
        if page_num not in self._thumbnail_frames:
            return

        frame = self._thumbnail_frames[page_num]

        if page_num in self._selected_pages:
            self._selected_pages.remove(page_num)
            frame.configure(bootstyle="default")
        else:
            self._selected_pages.add(page_num)
            frame.configure(bootstyle="success")

        self._update_selection_label()
        if self.on_selection_change:
            self.on_selection_change(self._selected_pages)

    def select_all(self) -> None:
        self._selected_pages = set(range(1, self._total_pages + 1))
        for frame in self._thumbnail_frames.values():
            frame.configure(bootstyle="success")
        self._update_selection_label()
        if self.on_selection_change:
            self.on_selection_change(self._selected_pages)

    def invert_selection(self) -> None:
        all_pages = set(range(1, self._total_pages + 1))
        self._selected_pages = all_pages - self._selected_pages
        for page_num, frame in self._thumbnail_frames.items():
            frame.configure(bootstyle="success" if page_num in self._selected_pages else "default")
        self._update_selection_label()
        if self.on_selection_change:
            self.on_selection_change(self._selected_pages)

    def clear_selection(self) -> None:
        self._selected_pages.clear()
        for frame in self._thumbnail_frames.values():
            frame.configure(bootstyle="default")
        self._update_selection_label()
        if self.on_selection_change:
            self.on_selection_change(self._selected_pages)

    def _update_selection_label(self) -> None:
        count = len(self._selected_pages)
        self.selection_label.config(text=f"已选: {count} 页")

    def get_selected_pages(self) -> list[int]:
        return sorted(self._selected_pages)

    def clear(self) -> None:
        # 取消所有进行中的任务
        for future in self._futures:
            future.cancel()
        self._futures.clear()

        self._filepath = ""
        self._total_pages = 0
        self._selected_pages.clear()
        self._thumbnails.clear()

        for frame in self._thumbnail_frames.values():
            frame.destroy()
        self._thumbnail_frames.clear()

        self.selection_label.config(text="已选: 0 页")

        for widget in self.grid_frame.winfo_children():
            widget.destroy()

        self.placeholder = ttk.Label(
            self.grid_frame, text="📄 选择 PDF 文件后显示页面缩略图",
            bootstyle="secondary", font=("微软雅黑", 11)
        )
        self.placeholder.pack(pady=50)

    def destroy(self) -> None:
        self._executor.shutdown(wait=False)
        super().destroy()


# =============================================================================
# 主应用
# =============================================================================

class PDFToolApp:

    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title(CONFIG.WINDOW_TITLE)
        self.root.geometry(CONFIG.WINDOW_SIZE)
        self.root.minsize(*CONFIG.WINDOW_MIN_SIZE)

        # 任务队列
        self._task_queue: queue.Queue = queue.Queue()

        # 状态变量
        self._merge_files: list[str] = []
        self._merge_page_counts: dict[str, int] = {}
        self._merge_busy: bool = False
        self._split_busy: bool = False
        self._rotate_busy: bool = False

        self._setup_ui()
        self._start_queue_poller()

    def _start_queue_poller(self) -> None:
        self._poll_queue()

    def _poll_queue(self) -> None:
        while not self._task_queue.empty():
            try:
                callback = self._task_queue.get_nowait()
            except queue.Empty:
                break
            try:
                callback()
            except Exception as e:
                self.root.after(0, lambda err=e: messagebox.showerror("错误", f"内部错误：{err}"))
        self.root.after(100, self._poll_queue)

    def _dispatch(self, fn: Callable, *args: Any, **kwargs: Any) -> None:
        if args or kwargs:
            self._task_queue.put(lambda: fn(*args, **kwargs))
        else:
            self._task_queue.put(fn)

    def _setup_ui(self) -> None:
        self.notebook = ttk.Notebook(self.root)
        self.notebook.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        # 创建各标签页
        self.merge_frame = ttk.Frame(self.notebook, padding=10)
        self.notebook.add(self.merge_frame, text="  合并 PDF  ")
        self._setup_merge_tab()

        self.split_frame = ttk.Frame(self.notebook, padding=10)
        self.notebook.add(self.split_frame, text="  拆分 PDF  ")
        self._setup_split_tab()

        self.rotate_frame = ttk.Frame(self.notebook, padding=10)
        self.notebook.add(self.rotate_frame, text="  旋转页面  ")
        self._setup_rotate_tab()

        # 全局快捷键
        self.root.bind("<Control-a>", self._on_ctrl_a)
        self.root.bind("<Control-m>", lambda _: self._execute_current_tab())
        self.root.bind("<Control-s>", lambda _: self._execute_current_tab())

    # =========================================================================
    # 合并功能
    # =========================================================================

    def _setup_merge_tab(self):
        frame = self.merge_frame

        # 左右分栏
        left_frame = ttk.Frame(frame, width=CONFIG.LEFT_PANEL_WIDTH)
        left_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(0, 10))
        left_frame.pack_propagate(False)

        right_frame = ttk.Frame(frame)
        right_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)

        # 标题
        SectionHeader(
            left_frame, "📄 合并多个 PDF 文件", "拖拽或添加文件，按顺序合并"
        ).pack(fill=tk.X, pady=(0, 10))

        # 文件列表
        list_container = LabelFrame(left_frame, text="  文件列表  ")
        list_container.pack(fill=tk.BOTH, expand=True, pady=5)

        list_border = ttk.Frame(list_container, bootstyle="default", relief="solid", borderwidth=1)
        list_border.pack(fill=tk.BOTH, expand=True)
        self._list_border = list_border

        self.merge_listbox = tk.Listbox(
            list_border, selectmode=tk.EXTENDED, height=10,
            font=("微软雅黑", 10), bg=CONFIG.COLOR_BG_LIGHT,
            selectbackground=CONFIG.COLOR_PRIMARY, selectforeground="white",
            activestyle="none", borderwidth=0, highlightthickness=0
        )
        scrollbar = ttk.Scrollbar(list_border, orient="vertical", command=self.merge_listbox.yview)
        self.merge_listbox.config(yscrollcommand=scrollbar.set)
        self.merge_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        # 拖拽绑定 - tkinterdnd2 方法
        self.merge_listbox.drop_target_register(DND_FILES)  # type: ignore[attr-defined]
        self.merge_listbox.dnd_bind("<<Drop>>", self._on_merge_drop)  # type: ignore[attr-defined]
        self.merge_listbox.dnd_bind("<<DropEnter>>", lambda e: self._list_border.configure(bootstyle="success"))  # type: ignore[attr-defined]
        self.merge_listbox.dnd_bind("<<DropLeave>>", lambda e: self._list_border.configure(bootstyle="default"))  # type: ignore[attr-defined]

        # 其他绑定
        self.merge_listbox.bind("<Motion>", self._on_merge_hover)
        self.merge_listbox.bind("<Leave>", lambda _: self._update_merge_preview())
        self.merge_listbox.bind("<Delete>", lambda _: self._merge_remove_selected())
        self.merge_listbox.bind("<Return>", lambda _: self._merge_pdfs())
        self.merge_listbox.bind("<<ListboxSelect>>", lambda _: self._on_merge_selection())

        # 按钮区域 - 第一行
        btn_frame1 = ttk.Frame(left_frame)
        btn_frame1.pack(fill=tk.X, pady=(10, 2))

        ttk.Button(btn_frame1, text="添加文件", bootstyle="info",
                   command=self._merge_add_files, width=10).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame1, text="添加文件夹", bootstyle="info-outline",
                   command=self._merge_add_folder, width=10).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame1, text="清空", bootstyle="secondary-outline",
                   command=self._merge_clear_list, width=8).pack(side=tk.RIGHT, padx=2)

        # 按钮区域 - 第二行
        btn_frame2 = ttk.Frame(left_frame)
        btn_frame2.pack(fill=tk.X, pady=(2, 5))

        ttk.Button(btn_frame2, text="🗑 删除", bootstyle="danger",
                   command=self._merge_remove_selected, width=10).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame2, text="↑ 上移", bootstyle="secondary-outline",
                   command=self._merge_move_up, width=8).pack(side=tk.RIGHT, padx=2)
        ttk.Button(btn_frame2, text="↓ 下移", bootstyle="secondary-outline",
                   command=self._merge_move_down, width=8).pack(side=tk.RIGHT, padx=2)

        # 预览信息
        preview_frame = LabelFrame(left_frame, text="  预览信息  ")
        preview_frame.pack(fill=tk.X, pady=5)
        self.merge_preview_info = tk.StringVar(value="")
        ttk.Label(preview_frame, textvariable=self.merge_preview_info,
                  bootstyle="info", font=("微软雅黑", 10)).pack(pady=5)

        # 操作按钮
        ttk.Separator(left_frame, orient="horizontal").pack(fill=tk.X, pady=8)
        ttk.Button(left_frame, text="✨ 合并 PDF", bootstyle="success",
                   command=self._merge_pdfs, width=18).pack(pady=5)

        # 进度条
        self.merge_progress = ProgressArea(left_frame, style="success-striped")

        # 右侧预览
        self.merge_preview = PDFPreviewPanel(right_frame, self)
        self.merge_preview.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

    def _on_merge_drop(self, event):
        self._list_border.configure(bootstyle="default")
        files = self.root.tk.splitlist(event.data)
        count = 0
        for f in files:
            f = os.path.abspath(f)
            if os.path.isfile(f) and f.lower().endswith(CONFIG.SUPPORTED_FORMAT):
                if f not in self._merge_files:
                    self._merge_files.append(f)
                    self.merge_listbox.insert(tk.END, self._build_display(f))
                    self._cache_pages(f)
                    count += 1
            elif os.path.isdir(f):
                count += self._add_folder_to_merge(f)
        if count > 0:
            self._update_merge_preview()

    def _on_merge_hover(self, event):
        idx = self.merge_listbox.nearest(event.y)
        if 0 <= idx < len(self._merge_files):
            self.merge_preview_info.set(f"📍 {self._merge_files[idx]}")

    def _on_merge_selection(self):
        sel = self.merge_listbox.curselection()
        if not sel:
            self.merge_preview.clear()
            return
        self.merge_preview.set_file(self._merge_files[sel[0]])

    def _cache_pages(self, filepath: str) -> int:
        if filepath in self._merge_page_counts:
            return self._merge_page_counts[filepath]
        try:
            r = PdfReader(filepath)
            n = len(r.pages)
            r.stream.close()
        except Exception:
            n = 0
        self._merge_page_counts[filepath] = n
        return n

    def _build_display(self, filepath: str) -> str:
        name = os.path.basename(filepath)
        try:
            r = PdfReader(filepath)
            pages = len(r.pages)
            r.stream.close()
        except Exception:
            pages = 0
        try:
            sz = _format_size(os.path.getsize(filepath))
        except Exception:
            sz = "?"
        return f"{name}  ({pages}页 | {sz})"

    def _merge_add_files(self):
        files = filedialog.askopenfilenames(title="选择 PDF 文件", filetypes=[("PDF 文件", "*.pdf")])
        if files:
            for fp in files:
                fp = os.path.abspath(fp)
                if fp not in self._merge_files:
                    self._merge_files.append(fp)
                    self.merge_listbox.insert(tk.END, self._build_display(fp))
                    self._cache_pages(fp)
            self._update_merge_preview()

    def _merge_add_folder(self):
        folder = filedialog.askdirectory(title="选择包含 PDF 的文件夹")
        if folder:
            count = self._add_folder_to_merge(folder)
            if count > 0:
                self._update_merge_preview()
            else:
                messagebox.showinfo("提示", "该文件夹中没有 PDF 文件")

    def _add_folder_to_merge(self, folder: str) -> int:
        pdf_files = []
        for root_dir, _, files in os.walk(folder):
            for f in files:
                if f.lower().endswith(CONFIG.SUPPORTED_FORMAT):
                    pdf_files.append(os.path.join(root_dir, f))

        pdf_files.sort(key=lambda x: os.path.basename(x).lower())

        count = 0
        for fp in pdf_files:
            fp = os.path.abspath(fp)
            if fp not in self._merge_files:
                self._merge_files.append(fp)
                self.merge_listbox.insert(tk.END, self._build_display(fp))
                self._cache_pages(fp)
                count += 1
        return count

    def _merge_remove_selected(self):
        selected = self.merge_listbox.curselection()
        if not selected:
            messagebox.showinfo("提示", "请先选中要删除的文件")
            return
        for i in reversed(selected):
            fp = self._merge_files[i]
            self.merge_listbox.delete(i)
            del self._merge_files[i]
            self._merge_page_counts.pop(fp, None)
        if self._merge_files:
            self.merge_listbox.selection_set(0)
            self._on_merge_selection()
        else:
            self.merge_preview.clear()
        self._update_merge_preview()

    def _merge_clear_list(self):
        if not self._merge_files:
            return
        if messagebox.askyesno("确认", "确定清空文件列表吗？"):
            self.merge_listbox.delete(0, tk.END)
            self._merge_files.clear()
            self._merge_page_counts.clear()
            self.merge_preview.clear()

    def _merge_move_up(self):
        self._merge_move(-1)

    def _merge_move_down(self):
        self._merge_move(1)

    def _merge_move(self, direction: int):
        sel = list(self.merge_listbox.curselection())
        if not sel or not self._merge_files:
            return
        n = len(self._merge_files)
        if direction == -1 and 0 in sel:
            return
        if direction == 1 and (n - 1) in sel:
            return

        indices = sel if direction == -1 else list(reversed(sel))
        swap_set = set()
        for i in indices:
            j = i + direction
            if 0 <= j < n and i not in swap_set and j not in swap_set:
                self._merge_files[i], self._merge_files[j] = self._merge_files[j], self._merge_files[i]
                swap_set.add(i)
                swap_set.add(j)

        self._rebuild_listbox(set(sel))
        self._on_merge_selection()
        self._update_merge_preview()

    def _rebuild_listbox(self, sel_indices: Optional[set[int]] = None):
        self.merge_listbox.delete(0, tk.END)
        for fp in self._merge_files:
            self.merge_listbox.insert(tk.END, self._build_display(fp))
        if sel_indices:
            for i in sorted(sel_indices):
                if 0 <= i < len(self._merge_files):
                    self.merge_listbox.selection_set(i)

    def _update_merge_preview(self):
        if not self._merge_files:
            self.merge_preview_info.set("")
            return

        total_pages = sum(self._merge_page_counts.get(f, 0) for f in self._merge_files)
        total_size = sum(os.path.getsize(fp) for fp in self._merge_files if os.path.exists(fp))

        info = f"文件: {len(self._merge_files)} | 页数: {total_pages} | 大小: {_format_size(total_size)}"
        self.merge_preview_info.set(info)

    def _merge_pdfs(self):
        if not self._merge_files:
            messagebox.showwarning("警告", "请先添加文件！")
            return
        if self._merge_busy:
            messagebox.showinfo("提示", "正在合并中，请稍候...")
            return

        output_path = filedialog.asksaveasfilename(
            title="保存合并文件", defaultextension=".pdf", filetypes=[("PDF 文件", "*.pdf")]
        )
        if not output_path:
            return

        self._merge_busy = True
        file_list = list(self._merge_files)
        threading.Thread(target=self._merge_worker, args=(output_path, file_list), daemon=True).start()

    def _merge_worker(self, output_path: str, file_paths: list[str]):
        try:
            self._dispatch(self.merge_progress.show)

            merger = PdfMerger()
            total = len(file_paths)
            for idx, path in enumerate(file_paths, 1):
                merger.append(path)
                pct = int(idx / total * 60)
                self._dispatch(lambda p=pct: self.merge_progress.update(p))

            self._dispatch(lambda: self.merge_progress.update(75, "正在写入文件..."))
            merger.write(output_path)
            merger.close()
            self._dispatch(lambda: self.merge_progress.update(100, "完成！"))
            self._dispatch(lambda: messagebox.showinfo("成功", f"已保存至：\n{output_path}"))
        except Exception as e:
            self._dispatch(lambda err=str(e): messagebox.showerror("错误", f"合并失败：\n{err}"))
        finally:
            self._dispatch(lambda: setattr(self, "_merge_busy", False))
            self._dispatch(self.merge_progress.hide)

    # =========================================================================
    # 拆分功能
    # =========================================================================

    def _setup_split_tab(self):
        frame = self.split_frame

        left_frame = ttk.Frame(frame, width=CONFIG.LEFT_PANEL_WIDTH)
        left_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(0, 10))
        left_frame.pack_propagate(False)

        right_frame = ttk.Frame(frame)
        right_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)

        # 标题
        SectionHeader(
            left_frame, "✂️ 拆分 PDF 文件", "按页码范围或固定页数拆分"
        ).pack(fill=tk.X, pady=(0, 10))

        # 拖拽区域
        drop_container = LabelFrame(left_frame, text="  选择文件  ")
        drop_container.pack(fill=tk.X, pady=5)

        self.split_drop = DropZone(drop_container, on_drop=self._on_split_drop)
        self.split_drop.pack(fill=tk.X, pady=5)

        btn_row = ttk.Frame(drop_container)
        btn_row.pack(pady=5)
        ttk.Button(btn_row, text="📁 选择文件", bootstyle="info",
                   command=self._split_browse, width=12).pack(side=tk.LEFT, padx=3)
        ttk.Button(btn_row, text="🔄 重置", bootstyle="danger",
                   command=self._split_reset, width=10).pack(side=tk.LEFT, padx=3)

        self.split_filepath = tk.StringVar()
        ttk.Entry(drop_container, textvariable=self.split_filepath,
                  state="readonly", font=("微软雅黑", 9)).pack(fill=tk.X, pady=(0, 5))

        # 拆分模式
        mode_frame = LabelFrame(left_frame, text="  拆分模式  ")
        mode_frame.pack(fill=tk.X, pady=5)

        self.split_mode = tk.StringVar(value="range")

        ttk.Radiobutton(mode_frame, text="按范围拆分（多文件）",
                        variable=self.split_mode, value="range",
                        bootstyle="primary").grid(row=0, column=0, sticky=tk.W, pady=4, padx=5)

        self.range_var = tk.StringVar(value="1-3,5,7-10")
        ttk.Entry(mode_frame, textvariable=self.range_var,
                  font=("微软雅黑", 10)).grid(row=1, column=0, padx=5, pady=3, sticky=tk.EW)
        ttk.Label(mode_frame, text="格式：1,3-5,7",
                  font=("微软雅黑", 8), bootstyle="secondary").grid(row=1, column=1, padx=5)

        ttk.Radiobutton(mode_frame, text="按每份页数拆分",
                        variable=self.split_mode, value="chunks",
                        bootstyle="primary").grid(row=2, column=0, sticky=tk.W, pady=4, padx=5)

        chunk_frame = ttk.Frame(mode_frame)
        chunk_frame.grid(row=3, column=0, sticky=tk.W, padx=5, pady=3)
        self.chunk_var = tk.IntVar(value=3)
        ttk.Spinbox(chunk_frame, from_=1, to=100,
                    textvariable=self.chunk_var, width=6).pack(side=tk.LEFT)
        ttk.Label(chunk_frame, text="页/份", font=("微软雅黑", 9)).pack(side=tk.LEFT, padx=5)

        ttk.Radiobutton(mode_frame, text="提取页面（单文件）",
                        variable=self.split_mode, value="extract",
                        bootstyle="primary").grid(row=4, column=0, sticky=tk.W, pady=4, padx=5)
        ttk.Label(mode_frame, text="将选中页面合并为一个 PDF",
                  font=("微软雅黑", 8), bootstyle="secondary").grid(row=5, column=0, sticky=tk.W, padx=5)

        mode_frame.columnconfigure(0, weight=1)

        # 输出预览
        preview_frame = LabelFrame(left_frame, text="  输出预览  ")
        preview_frame.pack(fill=tk.X, pady=5)

        self.split_preview = ttk.Treeview(preview_frame, columns=("file", "pages"),
                                           show="headings", height=3)
        self.split_preview.heading("file", text="文件名")
        self.split_preview.heading("pages", text="页数")
        self.split_preview.column("file", width=200)
        self.split_preview.column("pages", width=50, anchor="center")
        self.split_preview.pack(fill=tk.X, padx=5, pady=5)

        # 操作按钮
        ttk.Separator(left_frame, orient="horizontal").pack(fill=tk.X, pady=10)
        ttk.Button(left_frame, text="✂️ 开始拆分", bootstyle="warning",
                   command=self._split_pdf, width=18).pack(pady=5)

        self.split_progress = ProgressArea(left_frame, style="warning-striped")

        # 右侧预览
        self.split_preview_panel = PDFPreviewPanel(right_frame, self)
        self.split_preview_panel.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        self.split_thumbnail_grid = PageThumbnailGrid(
            right_frame, self, on_selection_change=self._on_extract_selection_change
        )
        self.split_thumbnail_grid.pack_forget()

        # 绑定更新
        self.split_mode.trace_add("write", lambda *_: self._on_split_mode_change())
        self.range_var.trace_add("write", lambda *_: self._update_split_preview())
        self.chunk_var.trace_add("write", lambda *_: self._update_split_preview())
        self.split_filepath.trace_add("write", lambda *_: self._on_split_file_change())

    def _on_split_drop(self, files: list[str]):
        if files:
            self.split_filepath.set(files[0])
            self._update_split_preview()

    def _on_split_mode_change(self):
        mode = self.split_mode.get()
        fp = self.split_filepath.get().strip()

        if mode == "extract":
            self.split_preview_panel.pack_forget()
            self.split_thumbnail_grid.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
            if fp and os.path.isfile(fp):
                self.split_thumbnail_grid.set_file(fp)
        else:
            self.split_thumbnail_grid.pack_forget()
            self.split_preview_panel.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
            if fp and os.path.isfile(fp):
                self.split_preview_panel.set_file(fp)

        self._update_split_preview()

    def _on_split_file_change(self):
        fp = self.split_filepath.get().strip()
        if fp and os.path.isfile(fp):
            mode = self.split_mode.get()
            if mode == "extract":
                self.split_thumbnail_grid.set_file(fp)
            else:
                self.split_preview_panel.set_file(fp)
        else:
            self.split_preview_panel.clear()
            self.split_thumbnail_grid.clear()
        self._update_split_preview()

    def _on_extract_selection_change(self, selected_pages: set[int]):
        self._update_split_preview()

    def _update_split_preview(self):
        self.split_preview.delete(*self.split_preview.get_children())
        fp = self.split_filepath.get().strip()
        if not fp or not os.path.isfile(fp):
            return

        try:
            reader = PdfReader(fp)
            total = len(reader.pages)
            reader.stream.close()
        except Exception:
            return

        base = os.path.splitext(os.path.basename(fp))[0]
        mode = self.split_mode.get()

        if mode == "range":
            range_str = self.range_var.get().strip()
            if range_str:
                parts = [p.strip() for p in range_str.split(",") if p.strip()]
                for idx, part in enumerate(parts, 1):
                    pages = self._parse_single_range(part, total)
                    if pages:
                        self.split_preview.insert("", tk.END, values=(f"{base}_{idx}.pdf", len(pages)))
        elif mode == "chunks":
            cs = self.chunk_var.get()
            if cs >= 1:
                nparts = (total + cs - 1) // cs
                for p in range(1, nparts + 1):
                    actual = min(cs, total - (p - 1) * cs)
                    self.split_preview.insert("", tk.END, values=(f"{base}_part_{p}.pdf", actual))
        elif mode == "extract":
            selected = self.split_thumbnail_grid.get_selected_pages()
            if selected:
                self.split_preview.insert("", tk.END, values=(f"{base}_extracted.pdf", len(selected)))

    def _split_browse(self):
        fp = filedialog.askopenfilename(title="选择要拆分的 PDF 文件", filetypes=[("PDF 文件", "*.pdf")])
        if fp:
            self.split_filepath.set(os.path.abspath(fp))

    def _split_reset(self):
        self.split_filepath.set("")
        self.split_mode.set("range")
        self.range_var.set("1-3,5,7-10")
        self.chunk_var.set(3)
        self.split_preview.delete(*self.split_preview.get_children())
        self.split_preview_panel.clear()
        self.split_thumbnail_grid.clear()

    def _parse_single_range(self, part: str, total_pages: int) -> Optional[list[int]]:
        part = part.strip()
        if not part:
            return None
        if "-" in part:
            try:
                start_str, end_str = part.split("-", 1)
                start, end = int(start_str), int(end_str)
                if start > end:
                    start, end = end, start
                start = max(1, start)
                end = min(total_pages, end)
                return list(range(start, end + 1)) or None
            except (ValueError, IndexError):
                return None
        else:
            try:
                page = int(part)
                return [page] if 1 <= page <= total_pages else None
            except ValueError:
                return None

    def _split_pdf(self):
        fp = self.split_filepath.get().strip()
        if not fp or not os.path.isfile(fp):
            messagebox.showwarning("警告", "请先选择有效的 PDF 文件！")
            return
        if self._split_busy:
            messagebox.showinfo("提示", "正在拆分中，请稍候...")
            return

        mode = self.split_mode.get()
        if mode == "extract":
            selected = self.split_thumbnail_grid.get_selected_pages()
            if not selected:
                messagebox.showwarning("警告", "请先选择要提取的页面！")
                return

        try:
            reader = PdfReader(fp)
            total_pages = len(reader.pages)
        except Exception as e:
            messagebox.showerror("错误", f"无法读取 PDF：\n{e}")
            return

        base_name = os.path.splitext(os.path.basename(fp))[0]
        output_dir = filedialog.askdirectory(title="选择保存文件夹")
        if not output_dir:
            return

        self._split_busy = True
        threading.Thread(target=self._split_worker,
                         args=(reader, total_pages, base_name, output_dir, mode), daemon=True).start()

    def _split_worker(self, reader, total_pages, base_name, output_dir, mode):
        try:
            self._dispatch(self.split_progress.show)
            if mode == "range":
                self._split_by_range(reader, total_pages, base_name, output_dir)
            elif mode == "chunks":
                self._split_by_chunks(reader, total_pages, base_name, output_dir)
            elif mode == "extract":
                self._split_extract(reader, base_name, output_dir)
        except Exception as e:
            self._dispatch(lambda err=str(e): messagebox.showerror("错误", f"拆分失败：\n{err}"))
        finally:
            self._dispatch(lambda: setattr(self, "_split_busy", False))
            self._dispatch(self.split_progress.hide)

    def _split_by_range(self, reader, total_pages, base_name, output_dir):
        page_input = self.range_var.get().strip()
        if not page_input:
            self._dispatch(lambda: messagebox.showerror("错误", "请输入页码范围！"))
            return

        parts = [p.strip() for p in page_input.split(",") if p.strip()]
        if not parts:
            self._dispatch(lambda: messagebox.showerror("错误", "页码格式不正确！示例：1,3-5,7"))
            return

        file_count = 0
        for idx, part in enumerate(parts, 1):
            page_numbers = self._parse_single_range(part, total_pages)
            if not page_numbers:
                continue
            writer = PdfWriter()
            for pn in page_numbers:
                writer.add_page(reader.pages[pn - 1])
            out_file = os.path.join(output_dir, f"{base_name}_{idx}.pdf")
            with open(out_file, "wb") as f:
                writer.write(f)
            file_count += 1
            pct = int(idx / len(parts) * 100)
            self._dispatch(lambda p=pct: self.split_progress.update(p))

        if file_count > 0:
            self._dispatch(lambda c=file_count, d=output_dir:
                           messagebox.showinfo("成功", f"已拆分为 {c} 个文件，保存在：\n{d}"))
        else:
            self._dispatch(lambda: messagebox.showerror("错误", "没有有效的页码范围"))

    def _split_by_chunks(self, reader, total_pages, base_name, output_dir):
        chunk_size = self.chunk_var.get()
        if chunk_size < 1:
            self._dispatch(lambda: messagebox.showerror("错误", "每份页数必须 >= 1"))
            return

        total_parts = (total_pages + chunk_size - 1) // chunk_size
        for part, start in enumerate(range(0, total_pages, chunk_size), 1):
            end = min(start + chunk_size, total_pages)
            writer = PdfWriter()
            for i in range(start, end):
                writer.add_page(reader.pages[i])
            out_file = os.path.join(output_dir, f"{base_name}_part_{part}.pdf")
            with open(out_file, "wb") as f:
                writer.write(f)
            pct = int(part / total_parts * 100)
            self._dispatch(lambda p=pct: self.split_progress.update(p))

        self._dispatch(lambda c=total_parts, d=output_dir:
                       messagebox.showinfo("成功", f"已拆分为 {c} 个文件，保存在：\n{d}"))

    def _split_extract(self, reader, base_name, output_dir):
        page_numbers = self.split_thumbnail_grid.get_selected_pages()
        if not page_numbers:
            self._dispatch(lambda: messagebox.showerror("错误", "请先选择要提取的页面"))
            return

        writer = PdfWriter()
        total = len(page_numbers)
        for idx, pn in enumerate(page_numbers, 1):
            writer.add_page(reader.pages[pn - 1])
            pct = int(idx / total * 100)
            self._dispatch(lambda p=pct: self.split_progress.update(p))

        output_path = os.path.join(output_dir, f"{base_name}_extracted.pdf")
        with open(output_path, "wb") as f:
            writer.write(f)

        self._dispatch(lambda p=output_path:
                       messagebox.showinfo("成功", f"已提取选中页面，保存在：\n{p}"))

    # =========================================================================
    # 旋转功能
    # =========================================================================

    def _setup_rotate_tab(self):
        frame = self.rotate_frame

        left_frame = ttk.Frame(frame, width=CONFIG.LEFT_PANEL_WIDTH)
        left_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(0, 10))
        left_frame.pack_propagate(False)

        right_frame = ttk.Frame(frame)
        right_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)

        # 标题
        SectionHeader(
            left_frame, "🔄 旋转 PDF 页面", "选择页面范围，设置旋转角度"
        ).pack(fill=tk.X, pady=(0, 10))

        # 拖拽区域
        drop_container = LabelFrame(left_frame, text="  选择文件  ")
        drop_container.pack(fill=tk.X, pady=5)

        self.rot_drop = DropZone(drop_container, on_drop=self._on_rotate_drop)
        self.rot_drop.pack(fill=tk.X, pady=5)

        btn_row = ttk.Frame(drop_container)
        btn_row.pack(pady=5)
        ttk.Button(btn_row, text="📁 选择文件", bootstyle="info",
                   command=self._rotate_browse, width=12).pack(side=tk.LEFT, padx=3)
        ttk.Button(btn_row, text="🔄 重置", bootstyle="danger",
                   command=self._rotate_reset, width=10).pack(side=tk.LEFT, padx=3)

        self.rot_filepath = tk.StringVar()
        ttk.Entry(drop_container, textvariable=self.rot_filepath,
                  state="readonly", font=("微软雅黑", 9)).pack(fill=tk.X, pady=(0, 5))

        self.rot_page_info = tk.StringVar(value="")
        ttk.Label(drop_container, textvariable=self.rot_page_info,
                  bootstyle="info", font=("微软雅黑", 9)).pack(pady=(0, 5))

        # 页面范围
        range_frame = LabelFrame(left_frame, text="  页码范围（留空 = 全部）  ")
        range_frame.pack(fill=tk.X, pady=5)
        self.rot_range_var = tk.StringVar(value="")
        ttk.Entry(range_frame, textvariable=self.rot_range_var,
                  font=("微软雅黑", 10)).pack(fill=tk.X, anchor=tk.W)
        ttk.Label(range_frame, text="格式：1,3-5,7",
                  font=("微软雅黑", 8), bootstyle="secondary").pack(pady=(3, 0))

        # 旋转角度
        angle_frame = LabelFrame(left_frame, text="  旋转角度  ")
        angle_frame.pack(fill=tk.X, pady=5)

        self.rot_angle = tk.IntVar(value=0)
        angles = [(0, "不旋转"), (90, "顺时针 90°"), (180, "旋转 180°"), (-90, "逆时针 90°")]
        for i, (val, txt) in enumerate(angles):
            ttk.Radiobutton(angle_frame, text=txt, variable=self.rot_angle,
                            value=val, bootstyle="primary"
                            ).grid(row=i // 2, column=i % 2, sticky=tk.W, padx=15, pady=5)

        # 逐页旋转设置
        per_page_frame = LabelFrame(left_frame, text="  逐页旋转设置  ")
        per_page_frame.pack(fill=tk.X, pady=5)

        self.rotation_tree = ttk.Treeview(per_page_frame, columns=("page", "angle"),
                                           show="headings", height=3)
        self.rotation_tree.heading("page", text="页码")
        self.rotation_tree.heading("angle", text="角度")
        self.rotation_tree.column("page", width=80, anchor="center")
        self.rotation_tree.column("angle", width=80, anchor="center")
        self.rotation_tree.pack(fill=tk.X, padx=5, pady=3)

        btn_row = ttk.Frame(per_page_frame)
        btn_row.pack(fill=tk.X, padx=5, pady=5)
        ttk.Button(btn_row, text="➕ 添加", bootstyle="info-outline",
                   command=self._add_rotation_setting, width=8).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_row, text="🗑 删除", bootstyle="secondary-outline",
                   command=self._remove_rotation_setting, width=8).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_row, text="清空", bootstyle="secondary-outline",
                   command=self._clear_rotation_settings, width=6).pack(side=tk.LEFT, padx=2)

        # 操作按钮
        ttk.Separator(left_frame, orient="horizontal").pack(fill=tk.X, pady=10)
        ttk.Button(left_frame, text="🔄 旋转并保存", bootstyle="warning",
                   command=self._rotate_pages, width=18).pack(pady=5)

        self.rot_progress = ProgressArea(left_frame, style="danger-striped")

        # 右侧预览
        self.rot_preview_panel = RotationPreviewPanel(right_frame, self)
        self.rot_preview_panel.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        # 实时旋转预览
        self.rot_angle.trace_add("write", lambda *_: self._update_rotation_preview())

    def _on_rotate_drop(self, files: list[str]):
        if files:
            self._set_rotate_file(files[0])

    def _update_rotation_preview(self):
        angle = self.rot_angle.get()
        self.rot_preview_panel.set_preview_rotation(angle)

    def _rotate_browse(self):
        fp = filedialog.askopenfilename(title="选择 PDF 文件", filetypes=[("PDF 文件", "*.pdf")])
        if fp:
            self._set_rotate_file(os.path.abspath(fp))

    def _rotate_reset(self):
        self.rot_filepath.set("")
        self.rot_range_var.set("")
        self.rot_angle.set(0)
        self._clear_rotation_settings()
        self.rot_preview_panel.clear()
        self.rot_page_info.set("")

    def _set_rotate_file(self, fp: str):
        self.rot_filepath.set(fp)
        self.rot_preview_panel.set_file(fp)
        try:
            r = PdfReader(fp)
            total = len(r.pages)
            r.stream.close()
            self.rot_page_info.set(f"共 {total} 页")
        except Exception:
            self.rot_page_info.set("")
        self._clear_rotation_settings()

    def _add_rotation_setting(self):
        range_str = self.rot_range_var.get().strip()
        angle = self.rot_angle.get()

        if not range_str:
            messagebox.showwarning("提示", "请输入页码范围")
            return

        try:
            r = PdfReader(self.rot_filepath.get())
            total = len(r.pages)
            r.stream.close()
        except Exception:
            messagebox.showerror("错误", "无法读取 PDF 文件")
            return

        pages = self._parse_page_ranges(range_str, total)
        if not pages:
            messagebox.showerror("错误", "页码格式不正确！\n示例：1,3-5,7")
            return

        for pn in pages:
            existing = self.rotation_tree.get_children()
            updated = False
            for item in existing:
                values = self.rotation_tree.item(item)["values"]
                if values and values[0] == pn:
                    self.rotation_tree.item(item, values=(pn, f"{angle}°"))
                    updated = True
                    break
            if not updated:
                self.rotation_tree.insert("", tk.END, values=(pn, f"{angle}°"))

    def _remove_rotation_setting(self):
        selected = self.rotation_tree.selection()
        if not selected:
            messagebox.showinfo("提示", "请先选中要删除的设置")
            return
        for item in selected:
            self.rotation_tree.delete(item)

    def _clear_rotation_settings(self):
        for item in self.rotation_tree.get_children():
            self.rotation_tree.delete(item)

    def _get_rotation_settings(self) -> dict[int, int]:
        settings = {}
        for item in self.rotation_tree.get_children():
            values = self.rotation_tree.item(item)["values"]
            if values:
                pn = int(values[0])
                angle_str = str(values[1]).replace("°", "")
                settings[pn] = int(angle_str)
        return settings

    def _parse_page_ranges(self, page_str: str, total_pages: int) -> Optional[list[int]]:
        page_str = re.sub(r"\s+", "", page_str)
        if not page_str:
            return None

        result: set[int] = set()
        parts = page_str.split(",")

        for part in parts:
            part = part.strip()
            if not part:
                continue
            if "-" in part:
                try:
                    start_str, end_str = part.split("-", 1)
                    start, end = int(start_str), int(end_str)
                    if start > end:
                        start, end = end, start
                    start = max(1, start)
                    end = min(total_pages, end)
                    result.update(range(start, end + 1))
                except (ValueError, IndexError):
                    return None
            else:
                try:
                    page = int(part)
                    if 1 <= page <= total_pages:
                        result.add(page)
                except ValueError:
                    return None

        return sorted(result) if result else None

    def _rotate_pages(self):
        fp = self.rot_filepath.get().strip()
        if not fp or not os.path.isfile(fp):
            messagebox.showwarning("警告", "请先选择有效的 PDF 文件！")
            return
        if self._rotate_busy:
            messagebox.showinfo("提示", "正在旋转中，请稍候...")
            return

        try:
            reader = PdfReader(fp)
            total = len(reader.pages)
        except Exception as e:
            messagebox.showerror("错误", f"无法读取 PDF：\n{e}")
            return

        per_page_settings = self._get_rotation_settings()

        if per_page_settings:
            page_numbers = list(per_page_settings.keys())
            angle = None
        else:
            range_str = self.rot_range_var.get().strip()
            if range_str:
                page_numbers = self._parse_page_ranges(range_str, total)
                if not page_numbers:
                    messagebox.showerror("错误", "页码格式不正确！\n示例：1,3-5,7")
                    return
            else:
                page_numbers = list(range(1, total + 1))
            angle = self.rot_angle.get()

            if angle == 0:
                messagebox.showwarning("提示", "请选择旋转角度（当前为「不旋转」）")
                return

        output = filedialog.asksaveasfilename(
            title="保存旋转后的文件", defaultextension=".pdf", filetypes=[("PDF 文件", "*.pdf")]
        )
        if not output:
            return

        self._rotate_busy = True
        threading.Thread(target=self._rotate_worker,
                         args=(reader, total, page_numbers, angle, output, per_page_settings),
                         daemon=True).start()

    def _rotate_worker(self, reader, total, page_numbers, angle, output_path, per_page_settings=None):
        try:
            self._dispatch(self.rot_progress.show)

            writer = PdfWriter()
            page_set = set(page_numbers)

            for i, page in enumerate(reader.pages, 1):
                if i in page_set:
                    if per_page_settings and i in per_page_settings:
                        page = page.rotate(per_page_settings[i])
                    elif angle is not None:
                        page = page.rotate(angle)
                writer.add_page(page)
                pct = int(i / total * 70)
                self._dispatch(lambda p=pct: self.rot_progress.update(p))

            self._dispatch(lambda: self.rot_progress.update(75, "正在写入文件..."))
            with open(output_path, "wb") as f:
                writer.write(f)
            self._dispatch(lambda: self.rot_progress.update(100, "完成！"))
            self._dispatch(lambda: messagebox.showinfo("成功", f"已保存至：\n{output_path}"))
        except Exception as e:
            self._dispatch(lambda err=str(e): messagebox.showerror("错误", f"旋转失败：\n{err}"))
        finally:
            self._dispatch(lambda: setattr(self, "_rotate_busy", False))
            self._dispatch(self.rot_progress.hide)

    # =========================================================================
    # 全局快捷键
    # =========================================================================

    def _on_ctrl_a(self, _event=None):
        current = self.notebook.select()
        if current == str(self.merge_frame):
            self.merge_listbox.selection_set(0, tk.END)

    def _execute_current_tab(self, _event=None):
        current = self.notebook.select()
        if current == str(self.merge_frame):
            self._merge_pdfs()
        elif current == str(self.split_frame):
            self._split_pdf()
        elif current == str(self.rotate_frame):
            self._rotate_pages()


# =============================================================================
# 入口
# =============================================================================

def main():
    root = TkinterDnD.Tk()
    _app = PDFToolApp(root)
    root.mainloop()


if __name__ == "__main__":
    main()
